...
export const updateProfileImage = (user: User) => {
  return (dispatch) => {
    dispatch({
      type: AuthActionTypes.UPDATE_USER_IMAGE_START,
    });
    const url = updateProfileImageUrl(localStorage.getItem("userId"));

    let formData = new FormData();
    for (let key in user) {
      if (key === "imageFile" && user[key] != null) {
    ...
    axios
      ...
      .then((res) => {
        let { data } = res;
        if (data) {
          updateProfileImageSuccess(dispatch, data);
        } else {
          updateProfileImageFail(dispatch, "There was an error connection");
        }
      })
      .catch((error) => {
        console.log(error.message);
        updateProfileImageFail(dispatch, "There was an error connection2");
      });
  };
};
const updateProfileImageFail = (dispatch, errorMessage) => {
  console.log(errorMessage);
  dispatch({
...
const updateProfileImageSuccess = (dispatch, data) => {
  dispatch({
