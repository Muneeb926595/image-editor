...
import { CustomeModal, Clickable, Icon, Avatar, Loader } from "@components";
...
const AddNewContact = () => {
  ...
  const doDebouncedAction = (event) => {
    ...
    debounce(dispatch(searchUsers(1, 50, searchText)), 1000);
  };
        ...
        <div className="w-full pt-4 pb-2 px-6">
          <form className="flex w-full rounded-full bg-gray-100 px-2 pr-4 items-center ">
            ...
            <input
              ...
              className="w-full ml-2 bg-gray-100 rounded-full  py-2 px-2 font-sans "
            />
          </form>
        </div>
        <div className="flex flex-col mt-8 overflow-scroll overflow-x-hidden app-scrollbar">
          ...
          {searchData?.length > 0 &&
            searchData.map((singleUser, index) => (
              ...
              >
                <Avatar uri={singleUser?.image} size="36" />
